...
### Поверхности
- **Glass**: `rgba(255, 255, 255, 0.6)` - Полупрозрачный светлый (карточки)
- **Glass Dark**: `rgba(255, 255, 255, 0.1)` - Полупрозрачный темный (badges темные)
- **Glass Light**: `rgba(255, 255, 255, 0.5)` - Полупрозрачный светлый (панели фильтров)
- **Glass Green**: `rgba(42, 150, 72, 0.5)` - Полупрозрачный зеленый (badges зеленые)
- **White**: `#FFFFFF`
...
#### FiltersPanel
Умная панель фильтров с логикой управления состоянием.

**Props:**
- `priceRange` - диапазон цен { min, max }
- `areaRange` - диапазон площади { min, max }
- `floorRange` - диапазон этажей { min, max }
- `roomsOptions` - опции комнат
- `featuresOptions` - опции особенностей

**События:**
- `@filter-change` - изменение фильтров
- `@reset` - сброс всех фильтров

**Использование:**
\`\`\`vue
<FiltersPanel
  :price-range="{ min: 10000000, max: 30000000 }"
  :area-range="{ min: 30, max: 200 }"
  :floor-range="{ min: 1, max: 25 }"
...
#### ButtonGroup
Презентационный компонент группы кнопок с единичным выбором.

**Props:**
- `modelValue` - выбранное значение (v-model)
- `options` - массив опций [{ label, value }]
...
#### RangeSlider
Презентационный компонент двойного слайдера.

**Props:**
- `modelValue` - значения [min, max] (v-model)
- `min` - минимальное значение
- `max` - максимальное значение
- `step` - шаг
- `prefix` - префикс значения
- `suffix` - суффикс значения
- `formatValue` - функция форматирования
...
<RangeSlider
  ...
  :step="100000"
  prefix="от "
  :format-value="(v) => (v/1000000).toFixed(1)"
/>
\`\`\`

#### FilterSection
Презентационная обертка для секции фильтра с заголовком.
